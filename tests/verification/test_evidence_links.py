...
def test_link_supports(tmp_path: Path) -> None:
    ...
    link = service.link_claim_evidence(
        "p", claim.claim_id, evidence, ClaimEvidenceRelationship.SUPPORTS, "direct"
    )
...
def test_link_contradicts_and_qualifies(tmp_path: Path) -> None:
    ...
    service.link_claim_evidence(
        "p", claim.claim_id, evidence[0], ClaimEvidenceRelationship.CONTRADICTS
    )
    service.link_claim_evidence(
        "p", claim.claim_id, evidence[1], ClaimEvidenceRelationship.QUALIFIES
    )
...
def test_link_invalid_evidence_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(EvidenceNotFoundError):
        service.link_claim_evidence(
            "p", claim.claim_id, "chunk_missing", ClaimEvidenceRelationship.SUPPORTS
        )
...
def test_link_missing_claim_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(ClaimNotFoundError):
        service.link_claim_evidence(
            "p", "claim_missing", evidence, ClaimEvidenceRelationship.SUPPORTS
        )


def test_link_claim_in_other_project_raises(tmp_path: Path) -> None:
    service, _, stack = build_service(tmp_path)
    claim = service.create_claim("projA", "surface code threshold")
    evidence = search_evidence(stack, "surface code", limit=1)[0]
    with pytest.raises(ClaimNotFoundError):
        service.link_claim_evidence(
            "projB", claim.claim_id, evidence, ClaimEvidenceRelationship.SUPPORTS
        )
...
def test_assess_evidence_maps_support_type(tmp_path: Path) -> None:
    ...
    assessment = service.assess_evidence("p", claim.claim_id, evidence)
