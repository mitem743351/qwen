...
CapabilityMode
    ...
    gateway_inference      # whether the gateway invokes a backend directly
...
**Inference owner: the local gateway.** The gateway owns the workflow and
explicitly invokes an inference backend through the `InferenceProvider`.
...
In this mode the gateway may control, **to the extent the provider supports it**:
...
The gateway must **never assume** a provider supports a parameter merely
because the abstract architecture defines it. `InferenceProvider` exposes
provider capabilities (see [`capability-negotiation.md`](capability-negotiation.md))
...
gateway for gateway-owned inference.
...
synchronization** between the Studio-native and gateway-owned paths. It is an
interface contract only; no protocol is implemented in this phase.
...
| Direct inference configuration | client-dependent / limited | Yes (provider-dependent) | Gateway-controlled for escalated tasks |
...
| Long-output continuation | client-dependent | Gateway-controlled | Gateway-controlled for escalated tasks |
...
- the gateway "controls Qwen" in STUDIO_NATIVE mode
...
- `GATEWAY_INFERENCE` — an explicit gateway-owned runtime (CLI/API/worker),
  with an `InferenceProvider` configured.
...
- Access to MCP tools does **not** grant permission to invoke arbitrary model
  backends.
- `STUDIO_NATIVE` must **not** implicitly gain access to gateway-owned API
  credentials.
